...
static int cdevice_init(struct dma_proxy_channel *pchannel_p, char *name)
{
	...
	pr_info("dma-proxy: [DEBUG] cdevice_init started for '%s'.\n", name);
...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	...
	pr_info("dma-proxy: [DEBUG] create_channel started for '%s'.\n", name);
	...
	pr_info("dma-proxy: [DEBUG] dma_request_chan for '%s' SUCCEEDED.\n", name);
...
{
	pr_info("dma-proxy: [DEBUG] Probe function has been successfully called! Device tree match is OK.\n");
	int rc, i;
	struct dma_proxy *lp;
	struct device *dev = &pdev->dev;

	pr_info("dma-proxy: [DEBUG] probe started.\n");
...
{
	tx_chan = dma_request_channel(mask, NULL, "dma17chan0");
   	rx_chan = dma_request_channel(mask, NULL, "dma17chan1");
}
