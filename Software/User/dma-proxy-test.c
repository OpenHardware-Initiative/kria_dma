...
const char *tx_channel_names[] = { "dma17chan0", /* add unique channel names here */ };
const char *rx_channel_names[] = { "dma17chan1", /* add unique channel names here */ };
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < TX_CHANNEL_COUNT; i++) {
		...
		printf("[DEBUG] Attempting to open TX device: %s\n", channel_name);
    		fflush(stdout);

		tx_channels[i].fd = open(channel_name, O_RDWR);
		if (tx_channels[i].fd < 1) {
			
			printf("[DEBUG] FAILED to open TX device: %s. Error: %s\n", channel_name, strerror(errno));
        		fflush(stdout);
		...
		printf("[DEBUG] Successfully opened TX device: %s\n", channel_name);
    		fflush(stdout);
